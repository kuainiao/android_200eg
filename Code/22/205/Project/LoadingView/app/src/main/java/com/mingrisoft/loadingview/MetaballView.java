...
public class MetaballView extends View {
    ...
    private ArrayList<Circle> circlePaths = new ArrayList<>();   //定义圆点路径的集合
...
//            radius1 *= scale1;
    ...
    private void startAnimation() {
        wa = new MoveAnimation();                //初始化移动的动画
        wa.setDuration(2500);                   //动画时间2.5s
        wa.setInterpolator(new AccelerateDecelerateInterpolator()); //设置不同速度的补值
        wa.setRepeatCount(Animation.INFINITE); //设置动画的重复
        wa.setRepeatMode(Animation.REVERSE);  //设置动画的重复模式
        startAnimation(wa);                   //开始动画
    }
