...
public class ActivityWakeUp extends MPermissionsActivity {
            ...
            public void onEvent(String name, final String params, byte[] data, int offset, int length) {
                ...
                try {
                    ...
                    if ("wp.data".equals(name)) { // 每次唤醒成功, 将会回调name=wp.data的时间, 被激活的唤醒词在params的word字段
                            ...
                            timer.schedule(new TimerTask() {
                                ...
                                public void run() {
                                    runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            if (powerLED.m_isOn) {  //如果闪光灯是开启的
                                                powerLED.turnOff();  //关闭闪光灯
                                            } else {  //否则
                                                powerLED.turnOn();  ///打开闪光灯
                                            }
                                        }
                                    });
                                }
                        ...
                        } else if (word.equals("出来聊聊")) {
                            // 通过包名获取要跳转的app，创建intent对象
                            Intent intent = getPackageManager().getLaunchIntentForPackage("com.mingrisoft.qrobot");
