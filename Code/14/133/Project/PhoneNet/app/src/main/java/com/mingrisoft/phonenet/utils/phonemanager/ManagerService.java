...
import com.mingrisoft.phonenet.utils.utils.WidgetUtils;
...
public class ManagerService extends Service {
    ...
    private void createFloatView() {
        ...
        tsp = (TSP) mFloatLayout.findViewById(R.id.wave_view_speed);
        mFloatLayout.measure(View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED), View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED));
        // 设置监听浮动窗口的触摸移动
        tsp.setOnTouchListener(new OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                // getRawX是触摸位置相对于屏幕的坐标，getX是相对于按钮的坐标
                wmParams.x = (int) event.getRawX() - tsp.getMeasuredWidth() / 2 - 20;
                wmParams.y = (int) event.getRawY() - tsp.getMeasuredHeight() / 2 - 80;// 减80为状态栏的高度
                mWindowManager.updateViewLayout(mFloatLayout, wmParams);// 刷新
                return false; // 此处必须返回false，否则OnClickListener获取不到监听
            }
        });
        tsp.setOnClickListener(new OnClickListener() {
    ...
    public void setSpeed(int str) {
//		mFloatView.setText(str.toString());
        tsp.setWaveHeight(str / 10);
    }
