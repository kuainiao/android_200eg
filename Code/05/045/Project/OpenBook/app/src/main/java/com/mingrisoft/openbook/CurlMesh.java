...
public class CurlMesh {
	...
	// 绘制多边形轮廓的标志。使用此标志在模拟器上崩溃
	// 由于我不知道的原因。把它留下它作为看到多边形
	// 纲要给出了良好的洞察力如何原始矩形被划分。
	...
	// 让我们尽量避免使用“new”。意思是我们介绍数组
	// 一旦在这里，并重用他们在运行时。没有真的有很大的影响
	// 但避免了一些垃圾收集的发生。
	...
	// 此网格的包围矩形。mrectagle [ 0 ] =左上角，
	// mrectangle [ 1 ] =左下，mrectangle [ 2 ] =右上和mrectangle [ 3 ]
	// 右下。
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius) {
		...
		// 旋转矩形的线条。根据排序后的顶点
		// 他们的X坐标我们不必担心指数的顶点
		// 0和1。但由于不准确的，它的可能的顶点3是不是
		// 顶点0的相对角。因此，我们计算距离
		// 顶点0到顶点2和3 -和改变线指数，如果需要的话。
		// 同样的顶点/线是在一个有序的第一个有X坐标
		// 至少后一个。这个属性是用来getintersections来
		// 看看是否有一个交集。
