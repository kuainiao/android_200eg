package com.mingrisoft.pedometer;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    CircularRingPercentageView progressCircle;      //定义步数进度条自定义控件
    TextView stepNumber;                            //显示步数的文本控件
    EditText change_step;                           //定义目标步数控件
    private int custom_steps;                      //用户的步数
    private int tarGet;                             //定义获取目标步数
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        change_step.addTextChangedListener(new TextWatcher() {                      //设置目标步数的监听器
            @Override
        ...
        SaveKeyValues.createSharePreferences(this);                                 //创建保存对象
        if (CURRENT_SETP != 0) {                                                       //如果不是第一次进入
            //获取保存的步数
